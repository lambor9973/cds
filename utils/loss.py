...
class DiscriminatorLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        Args:
            logits: Discriminator output [B, C_new]
            targets: Labels [B] (0 ~ C_new-1)
        Returns:
            Scalar loss value
        """
        target_onehot = torch.zeros_like(logits)
        target_onehot[torch.arange(logits.size(0)), targets] = 1.0

        prob = torch.clamp(logits, min=self.eps, max=1.0 - self.eps)

        loss = - (target_onehot * torch.log(prob) + (1 - target_onehot) * torch.log(1 - prob))
