...
def get_convnet(args, pretrained=False):
    ...
    if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
        ...
        if args["model_name"] == "adapter":
            ...
            adapter_placement = args.get("adapter_placement", "attention")
            adapter_interval = args.get("adapter_interval", 1)

            logging.info(f"Adapter placement strategy: {adapter_placement}")
            logging.info(f"Adapter interval: {adapter_interval}")
...
class SimpleVitNet(BaseNet):
    ...
    def generate_fc(self, in_dim, out_dim):
        ...
        # fc = GatedContinualLinear(in_dim,out_dim)
        return fc
    ...
    def forward(self, x,label=None, bcb_no_grad=False, fc_only=False):
        out = self.convnet(x)
