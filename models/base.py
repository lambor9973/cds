...
class BaseLearner(object):
    ...
    def _stage2_compact_classifier(self, task_size, ca_epochs=5):
        ...
        model = self._network if isinstance(self._network, nn.DataParallel) else self._network
        for p in model.fc.parameters():
            p.requires_grad = True

        run_epochs = ca_epochs
        crct_num = self._total_classes
        param_list = [p for p in model.fc.parameters() if p.requires_grad]
...
#  add other
    def _conpute_now_task_old_class_mean(self, data_manager):
        now_task_old_class_mean = np.zeros((self._total_classes - self._known_classes, self.feature_dim))
        now_task_old_class_covs = torch.zeros((self._total_classes - self._known_classes, self.feature_dim, self.feature_dim))
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            class_mean = np.mean(vectors, axis=0)
            class_cov = torch.cov(torch.tensor(vectors, dtype=torch.float64).T) + torch.eye(class_mean.shape[-1]) * 1e-3
            # class_cov = np.cov(vectosr.T)

            now_task_old_class_mean[class_idx - self._known_classes, :] = class_mean
            now_task_old_class_covs[class_idx - self._known_classes, ...] = class_cov
        return now_task_old_class_mean, now_task_old_class_covs

    def save_features(self,model, loader, target_classes,filename:str, class_means=None):
        """
        Save features for specified target classes.

        Args:
            model: Model for feature extraction.
            loader: Data loader.
            target_classes (list): List of target classes to save features for.
            filename (str): Name of the saved file.
            class_means (np.ndarray, optional): Class prototypes to save. Defaults to None.
        """
        model.eval()
        ...
        save_dir = "feature_visualizations/features"
        ...
        with torch.no_grad():
            for i, (_,inputs, targets) in enumerate(loader):
                ...
                features = outputs["sfeatures"]

                for feature,target in zip(features,targets):
                    if target.item() in target_classes:
                        collected_features.append(feature.cpu())
                        collected_labels.append(target.cpu())

        if not collected_features:
            print(f"Warning: No samples found for classes {target_classes}")

        features_tensor = torch.stack(collected_features)
        ...
        print(f"File saved to {save_path}, features shape: {features_tensor.shape}")

        if class_means is not None:
            data_to_save["class_means"] = torch.from_numpy(class_means)
            print(f"Class means added, shape: {class_means.shape}")
